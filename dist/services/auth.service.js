...
async function makeNewAccount(userId, password, email) {
  try {
    ...
    if (err instanceof PasswordHashingError) return "internalServerError";
    if (err instanceof UserAlreadyExistError) return "userIdAlreadyExist";
...
async function verifyTheSignIn(userId, password) {
  try {
    ...
    if (isMatch) return [isMatch, createAutheticationToken({ userId: userId })];
