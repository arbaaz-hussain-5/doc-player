...
const CommentController = {
  doTheComment: async function (req: Request, res: Response, next: Function) {
    try {
      await doTheComment(req.body.commentMessage, req.body.commentTo, req.body.commentBy);
      res.send('commented');
    } catch (err) {
      next(err);
    }
  ...
  doTheReply: async function (req: Request, res: Response, next: Function) {
    try {
      await replyToComment(req.body.commentMessage, req.body.commentTo, req.body.commentBy);
      res.send('replied');
    } catch (err) {
      next(err);
    }
  },
  deleteTheComment: async function (req: Request, res: Response, next: Function) {
    try {
