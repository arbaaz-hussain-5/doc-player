...
async function doTheComment(commentMessage: string, commentTo: string, commentBy: string): Promise<true> {
...
async function deleteTheComment(commentId: string): Promise<true> {
  ...
  try {
    await session.withTransaction(async () => {
      const commentToBeDeleted = await Comment.findOne({ _id: commentId }, {}, { session: session }).exec();
      ...
      await Comment.findByIdAndDelete(commentToBeDeleted.id, { session: session }).exec();
    });
    session.endSession();
    return true;
  } catch (err) {
...
async function replyToComment(commentMessage: string, commentTo: string, commentBy: string): Promise<true> {
  const session = await mongoose.startSession();
  try {
    await session.withTransaction(async () => {
      const [newComment] = await Comment.create(
...
async function deleteTheReply(commentId: string): Promise<true> {
  ...
  try {
    await session.withTransaction(async () => {
      ...
      await Comment.deleteOne({ id: commentToBeDeleted.id }, { session: session }).exec();
...
async function listTheComment(documentId: string): Promise<any> {
  try {
    const targetDocument = await Document.findById(documentId, 'comments').exec();
    if (!targetDocument) throw new DocumentNotFoundError('No Document');
...
async function listTheReply(commentId: string): Promise<any> {
  try {
    const targetComment = await Comment.findById(commentId, ' replyComments').exec();
    if (!targetComment) throw new CommentNotFoundError('no comment');
...
async function editTheComment(commentId: string, newMessage: string): Promise<boolean> {
  try {
...
export { doTheComment, deleteTheComment, replyToComment, deleteTheReply, listTheReply, listTheComment, editTheComment, editTheReply };
